    ...
    public void onClick(View view) {
        ...
        switch(view.getId()) {
            case R.id.popularButton:
                ...
                myURL = "https://api.themoviedb.org/3/movie/popular?api_key=<<API_KEY>>&language=en-US&page=1";
                Log.d(TAG, "onClick: popular button pressed with URL is " + myURL);
                intent.putExtra(EXTRA_MESSAGE,myURL);
                break;
            case R.id.playingButton:
                intent = new Intent(this,DisplayActivity.class);
                myURL = "https://api.themoviedb.org/3/movie/now_playing?api_key=<<API_KEY>>&language=en-US&page=1";
                Log.d(TAG, "onClick: playing button pressed with URL is " + myURL);
                intent.putExtra(EXTRA_MESSAGE,myURL);
                break;
            case R.id.ratedButton:
                intent = new Intent(this,DisplayActivity.class);
                myURL = "https://api.themoviedb.org/3/movie/top_rated?api_key=<<API_KEY>>&language=en-US&page=1";
                Log.d(TAG, "onClick: rated button pressed with URL is " + myURL);
                intent.putExtra(EXTRA_MESSAGE,myURL);
                break;
            case R.id.upcomingButton:
                intent = new Intent(this,DisplayActivity.class);
                myURL = "https://api.themoviedb.org/3/movie/upcoming?api_key=<<API_KEY>>&language=en-US&page=1";
